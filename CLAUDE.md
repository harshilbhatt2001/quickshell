...
- `Services/` — singletons (declared in `Services/qmldir`, each with `pragma Singleton`) that own all system state: `BatteryManager` (UPower), `NetworkManager`, `BluetoothManager`, `MprisManager`, `WorkspaceManager` (Hyprland), `NotificationManager`, `Time`, `CalendarManager` (Google Calendar via gcalcli: `agenda --tsv` to read, `add`/`edit`/`delete` to write; `editing` pins the island open and gives the bar keyboard focus), `CavaManager` (spectrum bars from `cava` raw output, only while the mpris toast is showing), and `IpcManager` (a Quickshell `IpcHandler` on target `"root"`, e.g. `quickshell ipc call root openLogoutMenu`). Adding a service means adding the file *and* registering it in `qmldir`.
